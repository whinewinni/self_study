        ...
        <tr>
            ...
            <td id="focusToday">
                ...
                <div id="appearTitle${i}" style="height: 50%">
                    <%--listall 사용하지 않고 직관적인 변수명을 사용
                    controller나 service에서도 변수명을 중복하게 사용하지 않는것이 좋다.--%>
                    <c:forEach var="calendarTable" items="${calendarTableList}" varStatus="j">
                        <%--if i is same with listAll.day, then shows the title--%>
                        <c:if test="${i==calendarTable.day}">
                            <%--${listAll}처럼 사용하는 것보다는 ${calendarTable}를 사용하여 domain임을 확인할 수 있기 해주는것 이 좋다.--%>
                            <div class="contentstitle" data-calendarnum="${calendarTable.calendarnum}" data-day="${calendarTable.day}">
                                <p id="showTitle${i}">${calendarTable.title}</p>
                                <%--name을 이용하여 controll해야함. eq로 접근하는것보다 직접적으로 접근하는 방법이 좋음--%>
                                <%--forech문에 varStatus를 사용해서 "content_${i}_${j.count}"과 같이 몇번째 값인지 확인할 수 있으며,
                                name을 고유값으로 사용할 수 있기때문에 직접적인 접근이 가능하다.--%>
                                <input type="hidden" name="content_${i}_${j.count}" value="${calendarTable.content}" />
                            </div>
                        </c:if>
                    </c:forEach>
                </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $("#contentsUpdateBtn").click(function () {
            var params = {
                year : $.trim($("#v_year").val()),
                month :$.trim($("#v_month").val()),
                day :$.trim($("#nowday").val()),
                title : $.trim($("#title").val()),
                content : $.trim($("#content").val()),
                calendarnum :$.trim($("#calendarnum").val())
            };

            $.ajax({
                /*url, type, dataType, data 순서로 정의해야함 */
                url:'/updateContent',
                type:'post',
                /*dataType을 지정해줘서 받아야함.*/
                dataType: 'text',
                /* data : params -> 변수들을 묶어서 보낼수 있음*/
                data : params,
        ...
        $("#contentsSaveBtn").click(function () {
            ...
            /*attr로 visible이나, display: none을 주는것보다는 show, hide로 지정해줌*/
        ...
        $("tbody tr td").hover(function () {
            ...
            if ($(this).text()!=""){
                /*style은 class를 만들어서 따로 관리하고, addClass와 removeClass를 사용하는 것이 좋다.*/
